...
const AppsPage = () => {
  ...
  const { getAppMode, getAllAppMode } = useAppModes();
  const workflowMode = getAppMode[AppMode.WORKFLOW];
  const chatMode = getAppMode[AppMode.CHAT];
  const allMode = getAllAppMode['all'];
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <Button variant={'primary'} size={'large'} onClick={() => setShowCreateDialog(true)}>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <StatCard icon={allMode.icon} title={allMode.name} value={`${stats.total} 个应用`} color="blue" />
          <StatCard icon={workflowMode.icon} title={workflowMode.name} value={`${stats.workflow} 个`} color="purple" />
          <StatCard icon={chatMode.icon} title={chatMode.name} value={`${stats.chatbot} 个`} color="green" />
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1 border border-[var(--border)] rounded-lg bg-background p-1">
              <button
        ...
        {viewMode === 'grid' ? (
          ...
          <div className="bg-background rounded-xl border border-[var(--border)]">
