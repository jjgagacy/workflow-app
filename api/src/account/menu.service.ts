...
export class MenuService {
  ...
  async getById(id: number): Promise<MenuEntity | null> {
    return this.menuRepository.findOne({ where: { id }, relations: { module: true, tenant: true } })
  }
   ...
   * @returns 菜单实体或null
   */
  async getByKey(key: string, tenantId: string): Promise<MenuEntity | null> {
    return this.menuRepository.findOneBy({ key, tenant: { id: tenantId } });
  }
  ...
  async create(dto: CreateMenuDto): Promise<MenuEntity | null> {
    ...
    const [existingByKey, existingByName] = await Promise.all([
      this.menuRepository.findOneBy({ key: dto.key, tenant: { id: dto.tenantId } }),
    ...
    if (existingByKey) {
      throw new BadRequestException(this.i18n.t('system.MENU_KEY_EXISTS', { args: { key: dto.key } }));
    }
    ...
    if (dto.moduleId) {
      module = await this.moduleRepository.findOneBy({ id: dto.moduleId, tenant: { id: dto.tenantId } });
      if (!module) {
        throw new BadRequestException(this.i18n.t('system.ID_NOT_EXIST', { args: { id: dto.moduleId } }));
      }
    }
  ...
  async update(dto: UpdateMenuDto): Promise<MenuEntity> {
    ...
    if (!menu) {
      throw new BadRequestException(this.i18n.t('system.MENU_NOT_EXIST', { args: { key: dto.key } }));
    }
    ...
    if (dto.parent !== undefined || dto.name !== undefined) {
      ...
      if (existingSameName) {
        throw new BadRequestException(this.i18n.t('system.CHILD_DUPLICATE', { args: { name: newName } }));
      }
    }
  ...
  async deleteById(id: number, tenantId: string): Promise<void> {
    const menu = await this.menuRepository.findOneBy({ id, tenant: { id: tenantId } });
    if (!menu) {
  ...
  async deleteByKey(key: string, parent: string, tenantId: string): Promise<void> {
    const menu = await this.menuRepository.findOneBy({ key, parent, tenant: { id: tenantId } });
    if (!menu) {
      throw new BadRequestException(this.i18n.t('system.MENU_NOT_EXIST', { args: { key } }));
    }
  ...
  async getMenus(tenantId: string): Promise<MenuInterface[]> {
    ...
    const menuRolesPromises = Array.from(menuMap.values()).map(async menuNode => {
      const menu = await this.getByKey(menuNode.key, tenantId);
  ...
  async getFlatMenus(args: Partial<GetMenuArgs>): Promise<MenuInterface[]> {
    ...
    const menuPromises = menus.map(async menu => {
      ...
      if (args.withModulePerm && menu.module?.key) {
        const perms = await this.moduleService.getModulePerms(menu.module.key, menu.tenant.id);
        if (perms) {
