...
export enum EmailType {
  ...
  CONFIRM_EMAIL_NEW = 'confirm_email_new',
}
...
export class MailI18nService {
  ...
  private initializeEmailTemplates(templates: Map<EmailType, Map<EmailLanguage, EmailContent>>): void {
    ...
    // 确认新邮箱模板
    templates.set(EmailType.CONFIRM_EMAIL_NEW, new Map([
      [EmailLanguage.EN_US, {
        subject: 'Confirm Your {applicationName} Email Change',
        templateName: 'confirm_email_new.hbs',
        context: {},
      }],
      [EmailLanguage.ZH_HANS, {
        subject: '确认您的 {applicationName} 邮箱修改',
        templateName: 'confirm_email_new.hbs',
        context: {},
      }]
    ]));
