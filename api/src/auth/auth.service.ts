...
export class AuthService {
    ...
    async validateUser(name: string, password: string): Promise<any> {
        ...
        if (!account) {
            throw new BadRequestGraphQLException(this.i18n.t('account.ACCOUNT_NOT_EXIST', { args: { name } }));
        }
        ...
        throw new BadRequestGraphQLException(this.i18n.t('auth.PASSWORD_INVALID'), { key: name });
    }

    async login(user: any): Promise<LoginResponse> {
        const userId = user?.id ?? 0;
        if (!userId) {
            throw new BadRequestGraphQLException(this.i18n.t('account.ACCOUNT_ID_NOT_EXISTS', { args: { name } }));
        }
