export function getEnumKey<T extends Record<string, string | number>>(
  ...
  const key = Object.keys(enumObj).find(
    (k) => enumObj[k] === value && isNaN(Number(k)), // 排除数字键（反向映射）
  );
  if (!key) {
...
export function getEnumKeySafe<T extends Record<string, string | number>>(
  enumObj: T,
  value: string | number,
): string | null {
...
export function getEnumValueSafe<T extends Record<string, string | number>>(
  enumObj: T,
  key: keyof T,
): string | number | null {
  const value = enumObj[key];
...
export class EnumUtils {
  static valueOf<T extends Record<string, string | number>>(
    enumObj: T,
    value: string | number,
  ): keyof T {
  ...
  static toValue<T extends Record<string, string | number>>(
    enumObj: T,
    key: keyof T,
  ): string | number {
  ...
  static safeValueOf<T extends Record<string, string | number>>(
    enumObj: T,
    value: string | number,
  ): keyof T | null {
  ...
  static safeToValue<T extends Record<string, string | number>>(
    enumObj: T,
    key: keyof T,
  ): string | number | null {
  ...
  static getKeySafe<T extends Record<string, string | number>>(
    enumObj: T,
    value: string | number,
  ): keyof T | null {
  ...
  static areAllEnumValues<T extends Record<string, string | number>>(
    enumObj: T,
    values: any[],
  ): values is T[keyof T][] {
  ...
  static validateEnumValues<T extends Record<string, string | number>>(
    enumObj: T,
    values: any[],
  ...
  static safeValidateEnumValues<T extends Record<string, string | number>>(
    enumObj: T,
    values: any[],
  ): boolean {
...
export class EnumConverter {
  static toEnum<T extends Record<string, string | number>>(
    ...
    throw new Error(
      `Invalid enum value: ${value} for enum: ${enumObj.constructor.name}`,
    );
  }

  static safeToEnum<T extends Record<string, string | number>>(
    enumObj: T,
    value: string | number,
  ): T[keyof T] | null {
  ...
  static toEnumArray<T extends Record<string, string | number>>(
    enumObj: T,
