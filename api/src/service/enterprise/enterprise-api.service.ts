...
export class EnterpriseApiService {
    ...
    private async sendRequest(
        ...
        const headers = {
            ...
            'Enterprise-API-Key': this.secretKey,
        };
        ...
        return this.httpService.request({
            ...
            catchError(error => {
                return throwError(() => new Error(`Request enterprise api error: ${error.message}`));
            }),
        );
    }
    ...
    async getData<T>(endpoint: string, data?: any, params?: any): Promise<T> {
        ...
        if (!response.success) {
            throw new Error(`Unable to retrieve enterprise data: ${response.message}`);
        }
    ...
    async postData<T>(endpoint: string, data?: any, params?: any): Promise<T> {
        ...
        const response = await lastValueFrom(request$);
        if (!response.success) {
            throw new Error(`Unable to post enterprise data: ${response.message}`);
        }
        return response.data;
    }
    ...
    async deleteData<T>(endpoint: string, data?: any, params?: any): Promise<T> {
        ...
        const response = await lastValueFrom(request$);
        if (!response.success) {
            throw new Error(`Unable to delete enterprise data: ${response.message}`);
        }
        return response.data;
    }
}